...
(function(){
    ...
    this.getState = function(row) {
        return this.states[row] || (this.$tokenizeRow(row),this.states[row]) || 'start';
    };

    this.$tokenizeRow = function(row) {
        return this.$tokenizeRows(row, row)[0];
    }
    /**
    ...
    this.$tokenizeRows = function(firstRow, lastRow) {
        ...
        if (firstRow > 0 && this.lines[firstRow - 1]) {
            ...
            state = this.states[this.lines.length-1];
        }
        ...
        for (var row=firstRow; row<=lastRow; row++) {
            if (!this.lines[row]) {
                ...
                rows.push(tokens.tokens);

                if (doCache) {
                    this.lines[row] = tokens.tokens;

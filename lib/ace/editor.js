...
(function(){
     ...
     * - dontSelect (Boolean): If `true`, does not select the entire document during replace
     *
     * Sets the current document to `val`.
     **/
    this.setValue = function(val, dontSelect) {
    ...
    this.onSelectionChange = function(e) {
        ...
        if (!session.$searchHighlight) {
            var highlight = new SearchHighlight(null, "ace_selected_word", "text");
            session.$searchHighlight = session.addDynamicMarker(highlight);
        }

        var re = this.$highlightSelectedWord && this.$getSelectionHighLightRegexp()
        this.session.$searchHighlight.setRegexp(re);
    };
    ...
    this.onCompositionStart = function(text) {
        this.renderer.showComposition(text);
    };
    ...
    this.find = function(needle, options, animate) {
        ...
        this.$search.set(options);
        var range = this.$search.find(this.session);
        if (range && !options.preventScroll)

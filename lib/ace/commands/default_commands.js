...
canon.addCommand({
    ...
    exec: function(env, args, request) {
        var line = parseInt(args.lineNumber);
        if (!isNaN(line)) {
            env.editor.gotoLine(line);
            // Always focus the editor in case we come from the command line.
            env.editor.focus();
        }
    }
});
    ...
    params: [
        {
            name: 'needle',
            type: 'text',
            description: 'String to find in the document.',
            defaultValue: null
        }
    ],
    exec: function(env, args, request) {
        env.editor.find(args.needle);
        // Always focus the editor in case we come from the command line.
        env.editor.focus();
    }
});
...
var replaceParams = [
    {
        name: 'needle',
        type: 'text',
        description: 'String to replace in the document.',
        defaultValue: null
    },
    {
        name: 'replacement',
        type: 'text',
        description: 'String that will be the replacement.',
        defaultValue: null
    }
]
canon.addCommand({
    ...
    exec: function(env, args, request) {
        env.editor.replace(args.replacement, {needle: args.needle});
        // Always focus the editor in case we come from the command line.
        env.editor.focus();
    }
});
canon.addCommand({
    ...
    params: replaceParams,
    exec: function(env, args, request) {
        env.editor.replaceAll(args.replacement, {needle: args.needle});
        // Always focus the editor in case we come from the command line.
        env.editor.focus();
    }
});

...
canon.addCommand({
    ...
    exec: function(env, args, request) { 
        ...
        var doc   = env.editor.getDocument();
        
		var lead = sel.getSelectionLead();
		var wordrange = doc.getWordRange(lead.row, lead.column);
		var current_word = lang.stringReverse(doc.getTextRange(wordrange)).trim();
        ...
        if(!doc.autoComRe || !doc.autoComText){
		    var w = lang.stringReverse(doc.getTextRange(wordrange)).trim();
			
			doc.autoComRe = new RegExp('([\\w\\d\\_]+'+ w +')[^\\w\\d]', 'g');
			
			var r = Range.fromPoints({'row':0,'column':0}, sel.getSelectionLead());
			doc.autoComText = lang.stringReverse(doc.getTextRange(r));
			doc.autoComMatchedWords = [];
			
			init = true;
        }
        
        if(!init && doc.autoComMatchedWords.length == 0) return false;
    	...
    	var inputWord = function(w){
			...
			doc.autoComMatchedWords.push(w);
		}
    	
    	while (!matched && (result = doc.autoComRe.exec(doc.autoComText))){
    		if(result.length == 2){
    			var match_word = lang.stringReverse(result[1]);
    			
    			if(doc.autoComMatchedWords.indexOf(match_word) == -1){

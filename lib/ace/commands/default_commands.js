...
canon.addCommand({
    ...
    exec: function(env, args, request) { 
        ...
        var w = lang.stringReverse(doc.getTextRange(wordrange));
    	
    	var r = Range.fromPoints({'row':0,'column':0}, sel.getSelectionLead());
    	var text = lang.stringReverse(doc.getTextRange(r));
    	var wr = new RegExp('([\\w\\d\\_]+'+ w +')[^\\w\\d]', '');
    	
    	var result;
    	if (result = wr.exec(text)){
    		if(result.length == 2){
    			sel.setSelectionRange(wordrange);
    			env.editor.onTextInput(lang.stringReverse(result[1]));
    		}
    	}
    }
});

...
var WORD_REGEX = /^[a-zа-яё]+/i,
    ...
    DISCUSSION_TEXT_ROW_REGEXP = /^\s*%\s*(.*)/;
    ...
    this.go = function(doc, doSpellcheck, doGetDiscussions) {
        ...
        for (var row = 0, linesLength = lines.length; row < linesLength; ++row) {
            ...
            for (var j = 0; j < tokenLength; ++j) {
                ...
                switch(token.type) {
                    case "comment":
                        doGetDiscussions && parseRowForDiscussions.call(this, line, row);
        ...
        function parseRowForDiscussions(tokenValue, row) {
            ...
            if (matchComment) {
                ...
                var matchTextRow = tokenValue.match(DISCUSSION_TEXT_ROW_REGEXP);
                if (matchTextRow) {
        ...
        function saveLastReadDiscussionComment() {
            if (this.myLastReadDiscussionComment != null) {
                if (this.myLastReadDiscussionComment.type == '*') {
                    this.myLastReadDiscussionComment.replies = []
                    this.myDiscussions.push(this.myLastReadDiscussionComment);
                }
                else if (this.myLastReadDiscussionComment.type == '^') {
                    this.myDiscussions[this.myDiscussions.length - 1].replies.push(this.myLastReadDiscussionComment);
                }
                this.myLastReadDiscussionComment = null;
            }
        }

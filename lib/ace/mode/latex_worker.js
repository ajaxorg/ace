...
(function() {
    ...
    this.changeOptions = function(newOptions) {
        ...
        var onSuccess = function() {
            cache.save(this.myOptions.language, dictionary);
        ...
        if (newOptions.enabled) {
            if (isNewLanguage) {
                dictionary = cache.load(this.myOptions.language);
                if (!dictionary) {
                    this.setDictionary(this.myOptions, onSuccess.bind(this), onError.bind(this));
                } else {
                    onSuccess.call(this);
                }
            } else {
    ...
    this.onUpdate = function() {
        ...
        if (this.doc.getValue()) {
            ...
            if (this.myOptions.enabled) {
                doSpellcheck = true;
                this.myParser.setSpellingCheckDictionary(dictionary);
            } else {
                doSpellcheck = false;
            ...
            this.myBgTokenizer.setDocument(this.doc);
            this.myParser.go(this.doc, doSpellcheck, doGetComments);

            doSpellcheck && this.sender.emit("spellcheck", this.myParser.getErrors());
            doGetComments && this.sender.emit("getComments", this.myParser.getComments());
        }
    };

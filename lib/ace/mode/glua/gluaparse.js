...
(function (root, name, factory) {
  ...
  var ast = exports.ast = {
    ...
    , emitStatement: function(eventName, eventArg) {
      return {
        name: eventName,
        arg: eventArg
      };
    ...
    , recordStatement: function(name, generics, properties) {
      return {
    ...
    , typedefStatement: function (name, oldRecordName, typeArgs) {
      return {
  ...
  function isKeyword(id) {
    switch (id.length) {
      ...
      case 3:
        return 'and' === id || 'end' === id || 'for' === id || 'not' === id || 'let' === id || 'var' === id;
      case 4:
        return 'else' === id || 'goto' === id || 'then' === id || 'type' === id || 'emit' === id;
      case 5:
  ...
  function parseStatement() {
    ...
    if (Keyword === token.type) {
      switch (token.value) {
        case 'local': case 'var': case 'let':    next(); return parseLocalStatement();
        ...
        case 'type':     next(); return parseRecordOfTypedefStatement();
        case 'emit':     next(); return parseEmitStatement();
      }
    }
  ...
  function parseEmitStatement() {
    var eventName = parseIdentifier();
    expect('(');
    var eventArg = parseExpression();
    expect(')');
    while(token.value === ';') {
      next();
    }
    return finishNode(ast.emitStatement(eventName, eventArg));
  }

  function parseRecordOfTypedefStatement() {
    if(token.value === '(') {
      // function call of type(expr)
      next();
      var args = [];
      while(token.value !== ')' && token.type !== EOF) {
    ...
    if(token.value === '{') {
      ...
      var properties = {};
      while(token.value !== '}' && token.type !== EOF) {
        var propName = parseIdentifier();
        expect(':');
        var propType = parseType();
        var propDefaultValue = null;
        if(token.value === 'default' || token.value === '=') {
          next();
          propDefaultValue = parseExpression();
        }
        properties[name] = {
          name: propName,
          type: propType,
      ...
      expect('}');
      return finishNode(ast.recordStatement(name, generics, properties));
    } else {
      var oldRecordName = parseType();
      var typeArgs = [];
      if(token.value === '<') {
        expect('<');
        while(token.value !== '>' && token.type !== EOF) {
      ...
      return finishNode(ast.typedefStatement(name, oldRecordName, typeArgs));
    }
  }
  ...
  function parseType() {
    ...
    if(token.value === '(') {
      ...
      expect(')');
      expect('=>');
      var funcRetType = parseType();
      return {
        type: 'function_type',
        argTypes: funcArgTypes,
        retType: funcRetType
      };
    } else if(token.type === BooleanLiteral || token.type === StringLiteral || token.type === NumericLiteral) {
      // literal value type
      type1 = {
        type: 'literal_type',
        literal: token.value
    ...
    if(token.value === '|') {
      next();
      var type2 = parseType();
      if(type2.type === 'union_type') {
        type2.items.unshift(type1);
        return type2;
      } else {
        return {type: 'union_type', items: [type1, type2]};
      }
    } else {
      return type1;
    }
  ...
  function parseTableConstructor() {
    ...
    while (true) {
      ...
      if ('}]'.indexOf(token.value) >= 0) break;
    }
    if(token.value === ']') {
      expect(']');
    } else {
  ...
  //          | functiondef | tableconstructor | lambda
  //
  //     prefixexp ::= (Name | '(' exp ')' ) { '[' exp ']'
  //          | '.' Name | ':' Name args | args }
  //

  function parseExpression(notIncludeTypeHint) {
    var expression = parseSubExpression(0, notIncludeTypeHint);
    return expression;
  }
  ...
  function parseSubExpression(minPrecedence, notIncludeTypeHint) {
    ...
    if (null == expression) {
      ...
      // PrefixExpression
      if (null == expression) {
        expression = parsePrefixExpression(notIncludeTypeHint);
      }
    }
    ...
    while (true) {
      ...
      next();
      var right = parseSubExpression(precedence, notIncludeTypeHint);
  ...
  function parsePrefixExpression(notIncludeTypeHint) {
    // @param notIncludeTypeHint not include ': type' part
    var base, name, marker
    ...
    // The prefix
    if (Identifier === token.type || token.value === 'type') {
      if(token.value === 'type') {
      ...
      if(token.value === '<') {
        next();
        var generics = [];
        // 用'<'符号的stack来不断读入identifer和'<','>'来判断是否是泛型record类型
        var leftBracketsStack = ['<'];
        do {
          if(token.value === '<') {
            leftBracketsStack.push('<');
            next();
            continue;
          } else if(token.value === '>') {
            ...
            var generic = parseType();
            generics.push(generic);
            if(token.value === ',') {
              next();
            }
          }
        } while(leftBracketsStack.length>0 && token.type !== EOF && (token.value === '<' || token.value === '>' || token.type===Identifier));
        // while(token.value !== '>' && token.type !== EOF) {
        //   var generic = parseType();
        //   generics.push(generic);
        //   if(token.value === ',') {
        ...
        //   }
        // }
        while(leftBracketsStack.length>0 && token.value === '>')
          next();
        if(leftBracketsStack.length===0)
          leftBracketsStack = true;
        if(leftBracketsStack.length>1)
          expect('>');
      }
  ...
  function parseCallExpression(base) {
    ...
    console.log('functioncall', token)
  ...
  function parsePrimaryExpression() {
    ...
    if (type & literals) {
      ...
      // 看是否是(args)=>ret或者(args) => do block end 或者 ( exp )
      
      pushLocation(marker);
      expect('(');
      var funcArgs = [];
      var cicleCount = 0;
      while(token.value !== ')' && token.type !== EOF && (++cicleCount<100)) {
        var argName = parseExpression(true);
        var arg = {
          name: argName
        };
        if(token.value === ':') {
      ...
      if(token.value === 'do') {
        ...
        expect('end');
        return finishNode({
          type: 'multiline_lambda',
          funcArgs: funcArgs,
          funcBody: funcBody
        });
      } else {
        var funcRetValue = parseExpression();
        return finishNode({
          type: 'lambda',
          funcArgs: funcArgs,
          funcRetValue: funcRetValue
        });
      }

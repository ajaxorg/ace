...
var TextInput = function(parentNode, host) {
    ...
    function sendText(valueToSend) {
        if (!copied) {
            ...
            if (value) {
                ...
                // if (!isFocused())
                //     return false;
                // this can happen only if vim mode tries to blur editor on textinput
                // (e.g. to focus commandline) but that is not a good reason to slow down all the input
                // it's better for vim mode to just throw exception or use timeout
            }
        }
    ...
    this.onContextMenu = function(mousePos, isEmpty){
        if (mousePos) {
            ...
            text.style.cssText = 
                'position:fixed; z-index:1000;width:20px;height:20px;opacity:0.1;' +
                'left:' + (mousePos.x - 10) + 'px; top:' + (mousePos.y - 10) + 'px;';
        }
        if (isEmpty)
            text.value='';
    };

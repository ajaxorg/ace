    ...
    return {
      ...
      find: function(back) {
        ...
        if (range && range.isEmpty()) {
          if (cm.ace.session.getLineLength(range.start.row) == range.start.column) {
  ...
  var Vim = function() { return vimApi; } //{
      ...
      processSearch: function(cm, vim, command) {
        ...
        switch (command.searchArgs.querySrc) {
          ...
          case 'wordUnderCursor':
            ...
            var query = cm.ace.session.doc.substring(cm.getLine(word.start.line), word.start.ch,
                word.end.ch);
    ...
    * match the input selection.
    ...
    function lineLength(cm, lineNum) {
      return cm.ace.session.getLineLength(lineNum);
    }
    function trim(s) {
      if (s.trim) {
        return s.trim();

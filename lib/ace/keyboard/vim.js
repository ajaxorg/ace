...
(function() {
  ...
  this.scanForBracket = function(pos, dir, _, options) {
    ...
    if (dir == 1) {
      var position = toAcePos(pos);
      var filteringIterator = this.ace.session.$newDefaultForwardIterator(position, /paren|text/);
      var m = this.ace.session.$findClosingBracket(re.slice(1, 2), position, filteringIterator);
    } else {
      var position = {row: pos.line, column: pos.ch + 1};
      var filteringIterator = this.ace.session.$newDefaultBackwardIterator(position, /paren|text/);
      var m = this.ace.session.$findOpeningBracket(re.slice(-2, -1), position, filteringIterator);
    }

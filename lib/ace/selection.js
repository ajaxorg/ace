...
(function() {
    ...
    this.$updateDesiredColumn = function() {
        if (this.$preventUpdateDesiredColumn)
            return;
    ...
    this.moveCursorLineStart = function() {
        ...
        // find the doc-position of the first character at the screen line.
        var startPos = this.session.screenToDocumentPosition(screenRow, 0);

        // if startPos is a real line (not wrapped one) move cursor to indent
        if (startPos.column == 0) {
            var indent = this.session.getLine(startPos.row).match(/^\s*/)[0].length;
            if (startPos.row != row || column > indent || column == 0)
                startPos.column = indent;
        }

        this.moveCursorTo(startPos.row, startPos.column);
    };
    ...
    this.moveCursorBy = function(rows, chars) {
        var lead = this.selectionLead;
        var screenPos = this.session.documentToScreenPosition(lead.row, lead.column);

        if (chars == 0 && rows < 0 && screenPos.row == 0) {
            return this.moveCursorTo(0, 0, lead.column != 0);
        } else if (chars == 0 && rows > 0 && lead.row == this.doc.getLength() - 1) {
            var col = this.session.getScreenLastRowColumn(screenPos.row);
            if (col == this.doc.getLine(lead.row).length)
                return this.moveCursorTo(lead.row, col, lead.column != col);
        }
    ...
    this.moveCursorTo = function(row, column, preventUpdateDesiredColumn) {
        ...
        this.$preventUpdateDesiredColumn = preventUpdateDesiredColumn;
        this.selectionLead.setPosition(row, column);
        this.$preventUpdateDesiredColumn = false;
    };

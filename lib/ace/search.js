...
(function() {
    ...
    **/
    this.findAll = function(session) {
        ...
        if (options.range) {
            ...
            var lines = this.session.getLines(range.start.row, range.end.row);
        } else
            var lines = this.session.doc.getAllLines();
        ...
        if (options.range) {
            ...
            var i = 0, j = ranges.length;
            while (i < j && ranges[i].start.column < startColumn)
                i++;

            while (i < j && ranges[j].end.column > endColumn)
                j--;
            return ranges.slice(i, j);
        }
    ...
    **/
    this.$matchIterator = function(session, options) {
        ...
        if (options.$isMultiLine) {
            ...
            var matchIterator = function(line, row, startIndex) {
                ...
                startIndex = (startIndex||0) + startLine.match(re[0])[0].length;
    ...
    this.$assembleRegExp = function(options) {
        if (typeof options.needle instanceof RegExp)
    ...
    this.$lineIterator = function(session, options) {
		...
		if (start.start)
            start = start[!options.skipCurrent != !backwards ? "end" : "start"];

...
var EgoEditSession = require("ace/edit_session/ego_edit_session");
   ...
   "test: find matching opening bracket" : function() {
    ...
    "test: wrapLine split function" : function() {
        ...
        function computeAndAssert(line, assertEqual, wrapLimit, tabSize) {
            ...
            var session = new EditSession(line);
            session.setTabSize(tabSize);
            session.setWrapLimit(wrapLimit);

            var tokens = session.$getDisplayTokens(line);
            var splits = session.$computeWrapSplits(tokens, wrapLimit, tabSize);

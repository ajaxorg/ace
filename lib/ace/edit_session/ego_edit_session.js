 ...
 */
function getScreenTabSize(screenColumn) {
 ...
 */
exports.getDisplayTokens = function(str, offset) {
    var arr = [];
    var tabSize;
    offset = offset || 0;
    ...
    for (var i = 0; i < str.length; i++) {
        ...
        // Tab
        if (c == 9) {
            tabSize = getScreenTabSize(arr.length + offset);
            arr.push(TAB);
...
exports.computeWrapSplits = function(tokens, wrapLimit, tabSize) {
    if (tokens.length == 0) {
        return [];
    }
...
function EgoEditSession() {
    ...
    this.$getDisplayTokens =  exports.getDisplayTokens;
    this.$computeWrapSplits = exports.computeWrapSplits;
...
/**
    ...
    this.$getStringScreenWidth = function(str, maxScreenColumn, screenColumn) {
        ...
        for (column = 0; column < str.length; column++) {
            ...
            // tab
            if (c == 9) {
                screenColumn += getScreenTabSize(screenColumn);
            }
            // full width characters
            else if (isFullWidth(c)) {
                screenColumn += 2;
    ...
    this.getScreenTabSize = getScreenTabSize;

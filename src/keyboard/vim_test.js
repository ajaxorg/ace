      ...
      return function(args) {
        ...
        for (var i = 0; i < args.length; i++) {
          ...
          var result = ch.length == 1
            ...
            : CodeMirror.Vim.handleKey(cm, args[i]);
          // ace_patch}
          if (!result && cm.state.vim.insertMode) {
            cm.replaceSelections(fillArray(args[i], cm.listSelections().length));
          }
        }
      };
    }
    function doInsertModeKeysFn(cm) {
      return function(args) {
        ...
        for (var i = 0; i < args.length; i++) {
          ...
          if (handled == "handled" && cm.state.vim.insertMode && args[i] != 'Esc') {
...
testVim('increment_hexadecimal', function(cm, vim, helpers) {
  ...
  helpers.doKeys('<C-x>');
  eq('0x00', cm.getValue());
}, { value: '0x0' });if (typeof module !== "undefined" && module === require.main) {
